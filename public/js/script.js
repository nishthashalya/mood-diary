...
function buildLegend() {
  ...
  settings.presets.forEach(p => {
    ...
    tag.addEventListener('click', () => {
      if (selectedDate) {
        // Apply preset to modal fields
        colorPicker.value = p.color;
        emojiPicker.value = p.emoji;
        scorePicker.value = p.score;
      }
    });
...
function renderPresetEditor() {
  ...
  settings.presets.forEach((p, idx) => {
    ...
    el.className = 'tag';
    el.innerHTML = `
      <input type="color" value="${p.color}" data-idx="${idx}" />
      <span style="min-width:28px;text-align:center">${p.emoji}</span>
      <span contenteditable data-idx="${idx}" style="outline:none">${p.label}</span>
    `;
    
    // Color picker change handler
    el.querySelector('input').addEventListener('change', e => {
      settings.presets[idx].color = e.target.value;
      persist();
      generateCalendar(currentYear, currentMonth);
      buildLegend();
    });
    
    // Label editor blur handler
    el.querySelector('[contenteditable]').addEventListener('blur', e => {
      settings.presets[idx].label = e.target.textContent.trim() || p.label;
      persist();
      buildLegend();
    });
...
function renderList() {
  ...
  const items = Object.entries(moodData)
    ...
    .filter(d => !q || 
      ...
      (d.tags || []).join(' ').includes(q.replace('#', ''))
    );

  entryListEl.innerHTML = '';
  
  if (!items.length) {
    entryListEl.innerHTML = '<div class="entry" style="justify-content:center;color:var(--muted)">No entries yet.</div>';
    return;
  }
...
function renderInsights() {
  ...
  if (!months.length) {
    insightFacts.innerHTML = '<span class="muted">No data yet.</span>';
    drawBar(chartMood, []);
    drawLine(chartScore, []);
    return;
  }
  ...
  const dist = Object.entries(byColor).map(([color, count]) => ({
    label: color, 
    value: count, 
    color
  }));
  ...
  data.forEach(d => {
    ...
    (byDay[day] || (byDay[day] = [])).push(d.score || 3);
  });
  const avg = Object.entries(byDay)
    .map(([day, arr]) => ({ x: +day, y: avgArr(arr) }))
    .sort((a, b) => a.x - b.x);
...
function drawBar(canvas, series) {
  ...
  series.forEach((s, i) => {
    ...
    ctx.fillText(String(s.value), x, h - 34);
  });
}
...
// UTILITY FUNCTIONS
function persist() {
  localStorage.setItem('moodDataV2', JSON.stringify(moodData));
...
toggleThemeBtn.addEventListener('click', () => {
  ...
  localStorage.setItem('themeV2', 
    document.documentElement.classList.contains('light') ? 'light' : 'dark'
  );
});

// Restore theme preference
if (localStorage.getItem('themeV2') === 'light') {
